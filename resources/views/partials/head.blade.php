    ...
    (function () {
        ...
        async function createEditor(host, inputId, wireModel) {
            ...
            try {
                ...
                if (window.editors[host.id]) {
                    try { await window.editors[host.id].destroy(); } catch (_) { }
                ...
                if (input) {
                    ...
                    if (form) {
                        form.addEventListener('submit', () => push(input, newEditor, host, wireModel), { capture: true });
                    }
                }
            } finally {
                window.__ckLocks[lockKey] = false;
        ...
        async function cleanup() {
            for (const id in window.editors) {
                try { await window.editors[id].destroy(); } catch (_) { }
