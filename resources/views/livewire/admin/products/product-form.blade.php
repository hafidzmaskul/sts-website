<div class="space-y-6 max-h-[70vh] overflow-y-auto px-1">
        ...
        <div class="space-y-4">
            <div>
                ...
                <input type="text" wire:model="brand_name"
                    class="w-full rounded-lg border px-3 py-2 text-black bg-white border-[#D2D2D2]">
            ...
            <div class="space-y-2 pt-4">
                <label class="flex items-center space-x-2 cursor-pointer">
                    <input type="checkbox" wire:model="is_sign_up_for_pricing"
                        class="rounded border-[#D2D2D2] text-indigo-600 shadow-sm focus:ring-indigo-500 bg-white">
                    <span class="text-sm text-gray-700">Sign Up For Pricing Only</span>
                </label>
                <label class="flex items-center space-x-2 cursor-pointer">
                    <input type="checkbox" wire:model="is_exclusive"
                        class="rounded border-[#D2D2D2] text-indigo-600 shadow-sm focus:ring-indigo-500 bg-white">
                    <span class="text-sm text-gray-700">Exclusive Product</span>
                </label>
            </div>
        </div>
    </div>
    ...
    <div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 gap-2 max-h-60 overflow-y-auto border rounded-lg p-3 bg-white border-[#D2D2D2]">
            @php
            ...
            @foreach($parents as $parent)
                <div class="col-span-full">
                    <label class="flex items-center gap-2 font-medium text-gray-900 bg-gray-50 p-1 rounded">
    ...
    <div class="space-y-6">
        <div wire:ignore x-data x-init="$nextTick(() => window.initCKEditor('key_feature_editor', 'key_feature_input', 'key_feature'))">
            ...
            <div id="key_feature_editor"
                class="min-h-[200px] rounded-lg border border-[#D2D2D2] ">
            </div>
    ...
    <div>
        ...
        @if(count($storedImages) > 0)
            <div class="space-y-4 mb-6">
                <h4 class="text-sm font-medium text-gray-600">Existing Images</h4>
                @foreach($storedImages as $index => $img)
                    <div class="flex items-center gap-4 p-4 border rounded-lg bg-gray-50 border-[#D2D2D2]">
                        ...
                        <div class="flex-1">
                            ...
                            <input type="number" wire:model="storedImages.{{ $index }}.sequence" class="w-24 rounded border px-2 py-1 text-sm text-black bg-white border-[#D2D2D2]">
                        </div>
                        <button type="button" wire:confirm="Are you sure?" wire:click="deleteImage({{ $img['id'] }})" class="text-red-500 hover:text-red-700">
                            Remove
                        </button>
                    </div>
                @endforeach
        ...
        <div class="space-y-4">
             <div class="flex justify-between items-center">
                <h4 class="text-sm font-medium text-gray-600">New Images</h4>
            ...
            @foreach($newImages as $index => $imgData)
                <div class="flex items-start gap-4 p-4 border rounded-lg bg-white border-[#D2D2D2]" wire:key="new-image-{{ $imgData['key'] }}">
                    <div class="flex-1">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Image File</label>
                        <input type="file" wire:model="newImages.{{ $index }}.image" class="block w-full text-sm text-black file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
                    ...
                    <div class="w-32">
                        ...
                        <input type="number" wire:model="newImages.{{ $index }}.sequence" class="w-full rounded-lg border px-3 py-2 text-black bg-white border-[#D2D2D2]">
                        @error("newImages.{$index}.sequence") <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                    </div>

                    <button type="button" wire:click="removeNewImage({{ $index }})" class="mt-7 text-red-500 hover:text-red-700 p-2">
            ...
            @if(empty($newImages) && empty($storedImages))
                <div class="text-center py-8 border-2 border-dashed border-[#D2D2D2] rounded-lg">
                    <p class="text-gray-500">No images added yet. Click "Add Image" to start.</p>
                </div>
            @endif
        </div>
    </div>
    ...
    <!-- SEO -->
    <div class="border-t pt-4 border-[#D2D2D2]">
        <h3 class="text-lg font-medium text-gray-900 mb-3">SEO Metadata</h3>
        <div class="space-y-4">
            <div>
            ...
            <div>
                ...
                <textarea wire:model="seo_description" rows="3"
                    class="w-full rounded-lg border px-3 py-2 text-black bg-white border-[#D2D2D2]"></textarea>
            </div>
            <div>
