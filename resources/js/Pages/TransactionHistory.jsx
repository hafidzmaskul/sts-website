...
export default function TransactionHistory() {
    ...
    return (
        <div className="min-h-screen flex flex-col bg-[#F3F3F3]">
            ...
            <main className="flex-1 container mx-auto px-2 sm:px-4 md:px-10 lg:px-20 py-6 md:py-10">
                <div className="max-w-6xl w-full mx-auto bg-white p-3 sm:p-6 rounded-lg shadow-sm">
                    <h1 className="text-xl sm:text-2xl font-semibold mb-4 sm:mb-6">My Transactions</h1>
                    ...
                    {isLoading ? (
                        ...
                        <div>
                            ...
                            <div className="md:hidden space-y-4">
                                {transactions.map((trx) => (
                                    <div key={trx.id} className="border rounded-lg bg-gray-50 px-3 py-4 shadow-sm flex flex-col gap-2">
                                        <div className="flex items-center justify-between">
                                            <span className="font-semibold text-sm text-gray-700">Date:</span>
                                            <span className="text-xs text-gray-600">{formatDate(trx.created_at)}</span>
                                        </div>
                                        <div className="flex items-center justify-between">
                                            <span className="font-semibold text-sm text-gray-700">Reference:</span>
                                            <span className="text-xs font-medium text-gray-800 break-words">{trx.invoice_code || trx.reference || `#${trx.id}`}</span>
                                        </div>
                                        <div className="flex items-center justify-between">
                                            <span className="font-semibold text-sm text-gray-700">Total Amount:</span>
                                            <span className="text-xs font-medium text-gray-800">{formatPrice(trx.total_amount)}</span>
                                        </div>
                                        <div className="flex items-center justify-between">
